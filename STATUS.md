...
| M5 | Generators & clear | ⬜ planned | split the single 🎲 into per-instrument **[generate · clear]** for 303 and drums; wire in the lesson-17 algos (Euclidean drums, scale-locked random-walk melody) each with its own small (collapsible) generator control group. _(Basic clear already shipped early: 303 `clear` button in the 303 card, drums `clear` via preset; 🎲 jam = 303 randomize in the app bar. M5 upgrades the generate side to the real algorithms + knobs.)_ |
...
- 2026-07-03: **persistence codec** (M3) = one bit-packed token for both
  ...
  🔗 button always mints a fresh link from current state). ⬇ export still M4.
